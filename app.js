...
function fireConfetti() {
    if (typeof confetti !== 'undefined') {
        confetti({ particleCount: 120, spread: 80, origin: { y: 0.6 }, colors: ['#4f46e5', '#10b981', '#f59e0b', '#ef4444', '#3b82f6'] });
    }
}
...
document.addEventListener('DOMContentLoaded', () => { applyTimerCollapse(); updateTimerDisplay(); });
...
function showAuthMessage(msg, colorClass = "text-red-500") {
    ...
    if(msgEl) { msgEl.textContent = msg; msgEl.className = `text-sm mt-2 ${colorClass}`; msgEl.classList.remove('hidden'); }
}
...
window.signUpWithEmail = async () => {
    ...
    if (error) showAuthMessage(error.message); else showAuthMessage("Account created! You are now logged in.", "text-green-500");
};
...
window.logout = async () => { await supabaseClient.auth.signOut(); window.location.href = 'index.html'; };
...
if(settingsForm) {
    settingsForm.addEventListener('submit', async (e) => {
        ...
        if (error) { msgEl.textContent = error.message; msgEl.className = "text-xs text-center mt-2 text-red-500"; } 
        else { msgEl.textContent = "Profile updated successfully!"; msgEl.className = "text-xs text-center mt-2 text-green-500"; document.getElementById('profilePassword').value = ''; }
...
window.parseSyllabusPDF = async () => {
    ...
    const skipSyllabusLessons = document.getElementById('skipSyllabusLessons').checked;
    ...
    try {
        ...
        statusMsg.textContent = skipSyllabusLessons ? "Successfully updated Course Description & Objectives!" : `Successfully imported metadata & ${addedCount} schedule items!`;
...
window.parseLessonsImage = async (inputElement) => {
    ...
    try {
        ...
        console.error(err);
        statusMsg.textContent = "OCR scan failed. Please check image quality.";
        statusMsg.className = "text-xs text-center mt-2 text-red-500";
    }
};
...
window.deleteAssignment = async (assignId, courseId) => { await supabaseClient.from('assignments').delete().eq('id', assignId); loadAssignments(courseId); };
...
if(customEventForm) {
    customEventForm.addEventListener('submit', async (e) => {
        ...
        await supabaseClient.from('custom_events').insert([{ user_id: currentUser.id, title: title, event_date: date, color: color }]);
        document.getElementById('evTitle').value = '';
        closeEventModal(); loadCalendarCourses();
    });
}
window.deleteCustomEvent = async (id) => { await supabaseClient.from('custom_events').delete().eq('id', id); loadCalendarCourses(); };
...
window.exportToICS = () => {
    ...
    const link = document.createElement('a'); link.href = window.URL.createObjectURL(blob); link.setAttribute('download', 'duevinci-schedule.ics');
