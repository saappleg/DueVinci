...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (useMock) {
      ...
      const { error } = await admin.from('canvas_connections').upsert({ user_id: user.id, canvas_domain: DEV_MOCK_CANVAS_DOMAIN, canvas_token: DEV_MOCK_CANVAS_TOKEN, updated_at: now }, { onConflict: 'user_id' })
