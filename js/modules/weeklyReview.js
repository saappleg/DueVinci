...
export async function renderWeeklyReview() {
    ...
    const dashboard = document.getElementById('dashboardWidgets') || document.getElementById('dashboardGrid');
    if (!dashboard) return;
    ...
    if (!card) { card = document.createElement('section'); card.id = 'weeklyReviewCard'; card.dataset.dashboardWidget = 'weekly_review'; dashboard.appendChild(card); }
    ...
    card.className = 'lg:col-span-2 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-brand-700 dark:bg-brand-800';
